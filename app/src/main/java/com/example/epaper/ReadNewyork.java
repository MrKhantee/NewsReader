...
public class ReadNewyork extends Activity {
	...
	final Context myApp = this; 
	   String url = "http://www.nytimes.com/?WT.z_jog=1";
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.news_newyorktimes);
        
        browser = (WebView)findViewById(R.id.webNewwork);
        browser.setWebViewClient(new MyBrowser());
        browser.getSettings().setLoadsImagesAutomatically(true);
        browser.getSettings().setJavaScriptEnabled(true);
        browser.setScrollBarStyle(View.SCROLLBARS_INSIDE_OVERLAY);
        browser.loadUrl(url);
       // bttn = (ImageButton) findViewById(R.id.playnav);
       new Sound().onStart();
    ...
    @SuppressLint("NewApi") private class Click extends AsyncTask<Void, Void, Void> {
		String desc;

		@Override
		protected void onPreExecute() {
			super.onPreExecute();
			mProgressDialog = new ProgressDialog(ReadNewyork.this);
		...
		protected Void doInBackground(Void... params) {
				...
				try {
					// Connect to the web site
					Document document = Jsoup.connect(browser.getUrl()).userAgent("Mozilla").get();
					// Using Elements to get the Meta data
					Elements description = document
							.select("div[class=story-body]");
